...
static int	partition(char *array[], int left, int right, int (*cmp)(char *,
					char *));
static void	quick_sort(char *array[], int left, int right, int (*cmp)(char *,
					char *));
			...
			char *))
{
	...
	if (left < right)
	{
		q = partition(array, left, right, cmp);
